...
def are_pmax_and_vla_identical(kpis):
    """Check if PMAX and PMAX VLA metrics are identical, indicating all PMAX campaigns are VLA"""
    if not (has_valid_data(kpis, 'pmax_') and has_valid_data(kpis, 'pmax_vla_')):
        return False
    
    # Compare key metrics
    metrics_to_compare = [
        ('pmax_impr', 'pmax_vla_impr'),
        ('pmax_clicks', 'pmax_vla_clicks'),
        ('pmax_cpc', 'pmax_vla_cpc'),
        ('pmax_conv', 'pmax_vla_conv'),
        ('pmax_cost_conv', 'pmax_vla_cost_conv')
    ]
    
    for pmax_key, vla_key in metrics_to_compare:
        if pmax_key in kpis and vla_key in kpis:
            if kpis[pmax_key] != kpis[vla_key]:
                return False
    
    # If we got here, all compared metrics are identical
    return True
...
def generate_email(kpis, report_month):
    ...
    # Check if PMAX and PMAX VLA metrics are identical
    pmax_vla_identical = are_pmax_and_vla_identical(kpis)
...
"""
    ...
    # Add PMAX section if valid data exists AND it's not identical to PMAX VLA
    if has_valid_data(kpis, 'pmax_') and not kpis.get('pmax_impr', '').startswith('[') and not pmax_vla_identical:
