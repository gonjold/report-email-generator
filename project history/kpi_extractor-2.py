...
- pmax_impr - Impressions from ALL PerformanceMax campaigns combined (total from the entire PMAX section)
- pmax_clicks - Clicks from ALL PerformanceMax campaigns combined (total from the entire PMAX section)
- pmax_cpc - Cost Per Click for ALL PerformanceMax campaigns combined (format $X.XX)
- pmax_conv - Conversions from ALL PerformanceMax campaigns combined
- pmax_cost_conv - Cost per Conversion for ALL PerformanceMax campaigns combined (format $X.XX)
- pmax_vla_impr - Impressions ONLY from the VLA campaigns (look for 'VLA_' prefix in campaign names)
- pmax_vla_clicks - Clicks ONLY from the VLA campaigns (look for 'VLA_' prefix in campaign names)
- pmax_vla_cpc - Cost Per Click ONLY from the VLA campaigns (format $X.XX)
- pmax_vla_conv - Conversions ONLY from the VLA campaigns (look for 'VLA_' prefix in campaign names)
- pmax_vla_cost_conv - Cost per Conversion ONLY from the VLA campaigns (format $X.XX)
...
6. The PerformanceMax metrics are in the PMAX section
7. The PerformanceMax w/ VLA metrics are in the PMAX_VLA section (look for campaigns starting with "VLA_")
...
12. CRITICAL: For PMAX_VLA metrics, ONLY count campaigns that start with "VLA_" - do not include other PMAX campaigns
13. PMAX metrics should be the TOTAL of all PMAX campaigns including VLA campaigns
...
def query_claude(api_key, document_text):
    ...
    try:
        ...
        4. Extract the correct cost per conversion for VLA campaigns
        """
...
def query_openai(api_key, document_text):
    ...
    try:
        ...
        4. Calculate the correct cost per conversion for VLA campaigns (it's different from regular PMAX)
        """
...
def query_deepseek(api_key, document_text):
    ...
    4. Calculate correct VLA cost per conversion from VLA-specific campaigns
    """
