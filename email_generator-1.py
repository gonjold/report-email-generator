...
--------------------------------
• Minimal Gmail‑friendly HTML (no colours, no fancy sizing)
...
def _section(title:str, rows:List[Tuple[str,str]], k:Dict[str,Any])->str:
    ...
    for lbl,key in rows:
        ...
        if val:
            lines.append(f"    <li><strong>{lbl}:</strong> {val}</li>")
    if not lines:
        return ""
    body="\n".join(lines)
...
def generate_email(kpis:Dict[str,Any], month:str)->Dict[str,str]:
    ...
    # build HTML
    html_parts=[f"""<div style="font-family:Arial, sans-serif; color:#000;">
<p><strong>SUBJECT:</strong> {month} MTD Digital Marketing Report – {store}</p>
<p>Hi {kpis.get('client_name','[Client Name]')},</p>
<p>Attached is the month‑to‑date performance report for <strong>{store}</strong>, covering <strong>{kpis.get('date_range','[Date Range]')}</strong>.</p>
<p><strong>KPI Breakdown by Channel:</strong></p>
"""]
    ...
    if kpis.get("has_bcdf"):
        rows=[("Impressions","bcdf_impr"),("Clicks","bcdf_clicks"),("Avg. CPC","bcdf_cpc"),
              ("VDP Views","bcdf_vdp"),("Conversions","bcdf_conv")]
        if _any_real(kpis,[r[1] for r in rows]):
            plain_lines.append(""); plain_lines.append("BUSINESS CENTER DIRECTED FUNDS (BCDF)")
            tactics=kpis.get("bcdf_tactics_organized",{}).get("tactics_list") or ", ".join(kpis.get("bcdf_tactics",[])) or "Unknown"
    ...
    plain_lines.extend(["","Thank you,","[Your Name]","[Your Title]","[Your Company]"])
